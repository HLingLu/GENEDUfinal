    ...
    def do(self):
        ...
        for num in range(0, total_pg):
            ...
            for d in doc['items']:
                ...
                # 商品網址
                item_url = f"https://shopee.tw/product/{shopid}/{productid}"
                # print(name)
                # print(item_url)

                # 商品內頁
                # get的網址
                re_item_url = f"https://shopee.tw/api/v4/item/get?itemid={productid}&shopid={shopid}"
                ...
                # 運費優惠內頁#
                # get的網址
                re_shipping_url = f"https://shopee.tw/api/v4/pdp/get_shipping?buyer_zipcode=&city=%E5%A4%A7%E5%AE%89%E5%8D%80&district=&itemid={productid}&shopid={shopid}&state=%E8%87%BA%E5%8C%97%E5%B8%82&town="
                ...
                # 每個細項名稱+賣場優惠卷+價格
                # voucher_discount = [(min_spend, discount_value)]
                voucher_discount = []
                # voucher_percent = [(min_spend, discount_percentage, max_discount)]
                voucher_percent = []
                for i, item in enumerate(imf['data']['shop_vouchers']):
                    if item['discount_value'] != None and item['discount_value'] > 0:
                        voucher_discount.append(
                            (int(item['min_spend']/100000), int(item['discount_value']/100000)))
                        ...
                        voucher_percent.append((int(item['min_spend']/100000), int(
                            100 - item['discount_percentage']), int(item['discount_cap']/100000)))
                ...
                if imf['data']['bundle_deal_info'] != None:
                    bundle.append(
                        imf['data']['bundle_deal_info']['bundle_deal_label'])
                ...
                for i, item in enumerate(imf['data']['models']):
                    ...
                    if len(imf['data']['models']) == 1:
                        ...
                        note+=f"商品名稱: {name}\n網址: {item_url}\n商品選項: {item['name']}\n"
                    note+=f"原價: {price}    折扣後價格: {lowest_price}    庫存: {item['stock']}    所使用優惠券: {voucher_use}\n"
                    note+="運費: \n"
                    if (len(ship) != 0):
                        for k in ship.keys():
                            note+=f"\t{k}: 運費{ship[k]}元\n"
                    note+="其他優惠: "
